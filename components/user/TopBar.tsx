...
export default function TopBar() {
  ...
  return (
    ...
    <div className="relative z-[10000] flex h-[40px] items-center bg-slate-900 text-[11px] text-slate-300 transition-all md:text-xs">
          ...
          <div className="relative">
            ...
            {langOpen && (
              <>
                ...
                <div className="absolute top-full right-0 z-50 mt-2 w-20 rounded-lg border border-slate-200 bg-white py-1 text-slate-800 shadow-xl">
                  {["ID", "EN", "JW"].map((code) => (
                    <button
                      key={code}
                      onClick={() => handleLangChange(code as any)}
                      className={`w-full py-1.5 text-center text-xs font-bold hover:bg-blue-50 ${lang === code ? "bg-blue-50 text-blue-600" : ""}`}
                    >
                      {code}
                    </button>
                  ))}
